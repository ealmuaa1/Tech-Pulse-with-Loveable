    ...
    async function fetchData() {
      ...
      try {
        ...
        if (profile?.favorite_topics) {
          preferences = profile.favorite_topics;
        ...
        // Step 2: Fix topic filter to be case-insensitive and match exactly
        let filtered = newsData;
        if (preferences.length > 0) {
          const prefLower = preferences.map((t) => t.toLowerCase());
          filtered = newsData.filter((item) => {
            const topic = item.topic?.toLowerCase() || "";
            const summary = item.summary?.toLowerCase() || "";
            const title = item.title?.toLowerCase() || "";
            return prefLower.some(
              (pref) =>
                topic.includes(pref) ||
                summary.includes(pref) ||
                title.includes(pref)
            );
          });
          // If no matches, show all
          if (filtered.length === 0) filtered = newsData;
        }
        setData(filtered);
      } catch (err) {
